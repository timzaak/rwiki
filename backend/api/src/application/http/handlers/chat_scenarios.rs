...
use rwiki_core::config::{ChannelConfig, ChannelsConfig, ChatConfig, LowRecallConfig};
...
/// Configured channel used by the low-recall integration scenarios. The seeded
/// published document belongs to this channel so that public `/api/chat` requests
/// pass the "channel has published documents" guard introduced by multi-channel
/// support.
const LOW_RECALL_CHANNEL_ID: &str = "help_center";
...
/// from the caller.
///
/// The state is channel-aware: `channels_config` contains `LOW_RECALL_CHANNEL_ID` and a
/// published document is seeded for that channel so public `/api/chat` requests
...
async fn build_low_recall_app_state(low_recall_config: Option<LowRecallConfig>) -> Arc<AppState> {
    ...
    // Seed a published document for the configured channel. This satisfies the
    // chat handler's channel-scope published-document guard.
    ...
    // Channel configuration that matches the seeded document.
    let mut channels = HashMap::new();
    channels.insert(
        LOW_RECALL_CHANNEL_ID.to_string(),
        ChannelConfig {
    ...
    let channels_config = ChannelsConfig { channels };
...
/// Read the `channel_id` of the most recently inserted `low_recall_records`
/// row whose `query` contains `query_substr`.
async fn read_low_recall_channel_id(state: &Arc<AppState>, query_substr: &str) -> Option<String> {
    let needle = format!("%{query_substr}%");
    state
        .sqlite
        .call(move |conn| -> Result<Option<String>, rusqlite::Error> {
            let mut stmt = conn.prepare(
                "SELECT channel_id FROM low_recall_records \
                     WHERE query LIKE ?1 \
        ...
        .await
        .expect("read low_recall channel_id")
}
...
// ===========================================================================
// Multi-channel public chat scenario tests (BE-T03)
// ===========================================================================
//
// Covers the channel-scoped behavior introduced by `support-multiple-website`:
// configured-channel validation, channel-scoped retrieval, channel-level prompt
...
/// Build a `ChannelsConfig` with three channels for chat scenario tests:
...
/// Build a minimal `AppState` for multi-channel chat scenarios.
///
/// The state is seeded with one published document for `CHANNEL_A` so that
/// `has_published_documents_for_channel` succeeds and `vector_store.is_empty()` is
/// false. The embeddings endpoint is mocked so that additional documents can be
/// indexed without hitting the real OpenAI API.
async fn build_channel_chat_app_state() -> Arc<AppState> {
    low_recall_ensure_sqlite_vec_loaded();
    ...
    let openai_client = rig::providers::openai::Client::builder()
        .api_key("sk-test-fake-key-for-channel-chat-tests-only")
        .base_url(embed_server.url());
    let embedding_model = openai_client
        .build()
        .expect("build openai client")
    ...
    let state = Arc::new(AppState {
        ...
        llm_client,
        llm_model: "test-model".to_string(),
        api_token: "test-api-token-channel-chat".to_string(),
        api_allowed_ip_ranges: Vec::new(),
        chat_config: ChatConfig::default(),
        static_dir: None,
        allowed_origins: vec![],
        ...
        mcp_config: None,
        channels_config: channel_chat_channels_config(),
        metrics: Arc::new(rwiki_core::infrastructure::metrics::RwikiMetrics::new()),
        session_count: Arc::new(std::sync::atomic::AtomicUsize::new(0)),
    });

    // Seed CHANNEL_A with a published document and indexed chunk so the channel has
    // usable data for the happy-path chat and retrieval scenarios.
    seed_channel_published_document(&state, CHANNEL_A, CHANNEL_A_DOC_ID, "channel A content").await;

    state
}

const CHANNEL_A_DOC_ID: &str = "11111111-1111-1111-1111-111111111111";

/// Seed an additional published document for a channel and index one chunk for it.
async fn seed_channel_published_document(
    state: &Arc<AppState>,
    channel_id: &str,
    doc_id: &str,
    content: &str,
    ...
    state
        ...
        .call(move |conn| -> Result<(), rusqlite::Error> {
            conn.execute(
                "INSERT INTO documents (id, file_name, status, created_at, channel_id) \
                 VALUES (?1, ?2, 'published', ?3, ?4)",
                rusqlite::params![doc_id_str, "test.md", created_at, channel_id],
            )?;
            Ok(())
        })
        .await
        .expect("insert channel document");
    ...
    state
        ...
        .await
        .expect("index channel document chunk");
}
...
// ---------------------------------------------------------------------------
// Configured-channel validation scenarios
// ---------------------------------------------------------------------------

// User Story: support-multiple-website — Widget and main-channel chat must declare
// a `channelId`. A request without one is rejected before any retrieval or LLM
// work is performed.
// Covers: `chat` handler calls `channels_config.require_configured` and maps the
// Empty error to a 400 Bad Request.

#[tokio::test]
async fn chat_missing_channel_id_returns_400() {
    ...
    assert_eq!(
        ...
        "chat without channelId must return 400"
    );

    let body = parse_json_body(resp.into_body()).await;
    assert!(
        body["message"].as_str().unwrap_or("").contains("channelId"),
        "error message must mention channelId, got {body}"
    );
}

// User Story: support-multiple-website — An explicitly provided but
// unconfigured `channelId` must be rejected so that stale or guessed identifiers
// cannot reach retrieval.
// Covers: `channels_config.require_configured` returns NotConfigured, mapped to
// 400 Bad Request.

#[tokio::test]
async fn chat_unconfigured_channel_id_returns_400() {
    let state = build_channel_chat_app_state().await;
    let app = create_api_routes(state);

    let req = channel_chat_request("hello", "session-unknown-channel", "unknown-channel");
    let resp = app.oneshot(req).await.expect("send request");

    assert_eq!(
        resp.status(),
        StatusCode::BAD_REQUEST,
        "chat with unknown channelId must return 400"
    );
...
// User Story: support-multiple-website — A configured channel with no published
...
// Covers: `chat` handler checks `has_published_documents_for_channel` and returns
// `当前频道没有可用文档` when the count is zero.

#[tokio::test]
async fn chat_configured_channel_without_published_docs_returns_503() {
    let state = build_channel_chat_app_state().await;
    let app = create_api_routes(state);
    ...
    assert_eq!(
        ...
        "chat for a configured channel with no published docs must return 503"
    );
...
// User Story: support-multiple-website — A valid, configured `channelId` with
...
// `has_published_documents_for_channel` for a public `/api/chat` request.

#[tokio::test]
async fn chat_valid_channel_id_succeeds_when_published_docs_exist() {
    let state = build_channel_chat_app_state().await;
    let app = create_api_routes(state);

    let req = channel_chat_request("hello", "session-valid-channel", CHANNEL_A);
    let resp = app.oneshot(req).await.expect("send request");
...
// ---------------------------------------------------------------------------
// Channel-scoped retrieval scenario
// ---------------------------------------------------------------------------

// User Story: support-multiple-website — Retrieval must only consider documents
// that belong to the requested channel and are published.
// Covers: `RetrievalScope::Channel` applied through `search_hybrid` filters out
// chunks from other channels.

#[tokio::test]
async fn channel_scoped_retrieval_returns_only_that_channels_published_documents() {
    let state = build_channel_chat_app_state().await;

    // Seed a second published document for CHANNEL_B.
    seed_channel_published_document(
        &state,
        CHANNEL_B,
    ...
    let scope = RetrievalScope::Channel(CHANNEL_A.to_string());
    let results = state
        .vector_store
        .search_hybrid("channel A content", 5, 1, 3, 12, 60, &scope)
        .await
        .expect("search_hybrid should succeed");

    assert!(
        !results.is_empty(),
        "channel-scoped search for CHANNEL_A must return at least one result"
    );
    assert!(
        results.iter().all(|r| r.document_id == CHANNEL_A_DOC_ID),
        "every retrieved chunk must belong to CHANNEL_A; got document_ids: {:?}",
        results.iter().map(|r| &r.document_id).collect::<Vec<_>>()
    );
}

// ---------------------------------------------------------------------------
// Channel-level system prompt scenarios
// ---------------------------------------------------------------------------

// User Story: support-multiple-website — A channel may override the global system
// prompt; channels without an override must continue to use the global prompt.
// Covers: `ChannelsConfig::resolved_system_prompt` returns the channel override when
// present and non-empty, and falls back to the global prompt otherwise.

#[test]
fn channel_specific_system_prompt_overrides_global_and_unconfigured_channel_falls_back() {
    let mut channels = HashMap::new();
    channels.insert(
        CHANNEL_A.to_string(),
        ChannelConfig {
    ...
    assert_eq!(
        channels_config.resolved_system_prompt(Some(CHANNEL_A), global_prompt),
        "Site A override prompt.",
        "CHANNEL_A must use its configured system prompt"
    );
    assert_eq!(
        channels_config.resolved_system_prompt(Some(CHANNEL_B), global_prompt),
        global_prompt,
        "CHANNEL_B without an override must fall back to the global prompt"
    );

    // The preamble builder receives the resolved prompt, so the final prompt
    // embeds the channel-specific text.
    let preamble = build_preamble(
        channels_config.resolved_system_prompt(Some(CHANNEL_A), global_prompt),
        None,
        "RAG context",
    ...
    assert!(
        ...
        "preamble must start with the resolved channel-specific prompt"
    );
}
...
// different channels must not share conversation history, otherwise context from
// one channel would leak into another.
// Covers: `session_key(Some(channel_id), session_id)` prefixes the storage key
// with the channel id, so identical session ids produce distinct map keys.
...
fn identical_session_id_in_different_channels_uses_distinct_storage_keys() {
    ...
    assert_ne!(
        key_a, key_b,
        "storage keys for the same session id in different channels must differ"
    );
    assert!(
        key_a.contains(CHANNEL_A),
        "CHANNEL_A storage key must contain the channel id"
    );
    assert!(
        key_b.contains(CHANNEL_B),
        "CHANNEL_B storage key must contain the channel id"
    );
}

// User Story: support-multiple-website — A session inserted for CHANNEL_A must not
// be visible when the handler looks up the same session id for CHANNEL_B.
// Covers: the in-memory session map is keyed by `session_key`, so cross-channel
// lookups miss.

#[tokio::test]
async fn cross_channel_session_lookup_does_not_share_history() {
    let state = build_channel_chat_app_state().await;
    ...
    assert_eq!(
        ...
        1,
        "CHANNEL_A session history must be preserved"
    );
}
...
// User Story: support-multiple-website — When a channel-scoped chat query triggers
// the low-recall bypass, the written record must carry the current `channelId` so
// operators can query low-recall records per channel.
// Covers: `chat_inner` writes `channel_id` into `low_recall_records.channel_id` for
// `RetrievalScope::Channel`.

#[tokio::test]
async fn low_recall_channel_scoped_chat_records_channel_id() {
    let state = test_app_state_with_low_recall(0.3).await;
    let app = create_api_routes(state.clone());

    let marker = "zzzzz-low-recall-channel-attribution-marker";
    let req = low_recall_chat_request(marker, "session-low-recall-channel-attribution");
    let resp = app.oneshot(req).await.expect("send request");
    ...
    let channel_id = {
        ...
        for _ in 0..20 {
            tokio::time::sleep(std::time::Duration::from_millis(25)).await;
            maybe = read_low_recall_channel_id(&state, marker).await;
            if maybe.is_some() {
                break;
            }
        }
