...
def create_container(args: list[str]) -> str | None:
    """Create a container without starting it. Returns container ID or None."""
    result = _run(["create", *args])
    if result.returncode != 0:
        _log_error("create", result.stderr)
        return None
    return result.stdout.strip()


def copy_into_container(container_id: str, src: str, dst: str) -> bool:
    """Copy a file or directory from host into a container."""
    result = _run(["cp", src, f"{container_id}:{dst}"])
    if result.returncode != 0:
        _log_error("cp", result.stderr)
    return result.returncode == 0


def start_container(container_id: str) -> bool:
    """Start a created container."""
    result = _run(["start", container_id])
    if result.returncode != 0:
        _log_error("start", result.stderr)
    return result.returncode == 0
