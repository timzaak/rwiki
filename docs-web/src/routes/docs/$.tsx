...
const nonDefaultLocales = i18n.languages.filter(
...
export const Route = createFileRoute("/docs/$")({
  ...
  loader: async ({ params }) => {
    ...
    // Redirect old locale-prefixed URLs (/docs/zh/...) to new pattern (/zh/docs/...)
    if (
      slugs.length > 0 &&
      nonDefaultLocales.includes(slugs[0] as (typeof nonDefaultLocales)[number])
    ) {
      const locale = slugs[0];
      const rest = slugs.slice(1).join("/");
      throw redirect({ href: `/${locale}/docs/${rest}` });
    }
    // /docs → redirect to getting-started
    if (slugs.length === 0) {
      throw redirect({ href: "/docs/getting-started" });
    }
