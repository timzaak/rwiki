...
6. **上下文组装**：检索结果格式化为结构化 XML，包裹在 `<documents>` 块中。每个结果生成一个 `<document index="N">` 元素，包含 `<title>`、`<section>`、`<link>`、`<locale>`、`<content>` 标签。这给 LLM 提供了清晰的、机器可解析的上下文，支持按编号引用来源
7. **流式生成**：通过 SSE 事件推送到前端，事件类型包括 `session`、`chunk`、`done`、`error`。默认系统提示词指示 LLM 使用 `[Source N]` 格式引用来源，其中 N 对应上下文 XML 中的 document index
